...
func (g *FilterGraph) NewFilterContext(f *Filter, name string) (*FilterContext, error) {
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
	c := C.avfilter_graph_alloc_filter(g.c, f.c, cname)
	if c == nil {
...
func (g *FilterGraph) Link(src *FilterContext, srcPad int, dst *FilterContext, dstPad int) error {
	return newError(C.avfilter_link(src.c, C.uint(srcPad), dst.c, C.uint(dstPad)))
}

// https://ffmpeg.org/doxygen/8.0/group__lavfi.html#ga1896c46b7bc6ff1bdb1a4815faa9ad07
func (g *FilterGraph) CreateFilter(filter *Filter, name, args string) (*FilterContext, error) {
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
	var cargs *C.char
	if args != "" {
	...
	if err := newError(C.avfilter_graph_create_filter(&c, filter.c, cname, cargs, nil, g.c)); err != nil {
		return nil, err
	}
	fc := newFilterContext(c)
	g.fcs = append(g.fcs, fc)
...
// https://ffmpeg.org/doxygen/8.0/group__lavfi.html#ga1896c46b7bc6ff1bdb1a4815faa9ad07
func (g *FilterGraph) QueueCommand(target, cmd, args string, f FilterCommandFlags, ts float64) error {
	targetc := C.CString(target)
	defer C.free(unsafe.Pointer(targetc))
	cmdc := C.CString(cmd)
	defer C.free(unsafe.Pointer(cmdc))
	argsc := C.CString(args)
	defer C.free(unsafe.Pointer(argsc))
	return newError(C.avfilter_graph_queue_command(g.c, targetc, cmdc, argsc, C.int(f), C.double(ts)))
}

// https://ffmpeg.org/doxygen/8.0/group__lavfi.html#ga1896c46b7bc6ff1bdb1a4815faa9ad07
func (g *FilterGraph) RequestOldest() error {
	return newError(C.avfilter_graph_request_oldest(g.c))
}
...
// SetAutoConvert sets auto conversion flags for the filter graph
// https://ffmpeg.org/doxygen/8.0/group__lavfi.html#ga7b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b
func (g *FilterGraph) SetAutoConvert(flags uint) {
	C.avfilter_graph_set_auto_convert(g.c, C.uint(flags))
}
