...
func TestFrameFlags(t *testing.T) {
	fs := NewFrameFlags(FrameFlag(1))
	require.True(t, fs.Has(FrameFlag(1)))
	fs = fs.Add(FrameFlag(2))
	require.True(t, fs.Has(FrameFlag(2)))
	fs = fs.Del(FrameFlag(2))
	require.False(t, fs.Has(FrameFlag(2)))
}
