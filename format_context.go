...
// https://ffmpeg.org/doxygen/8.0/group__lavf__mux.html#ga18b7b10bb5b94524842de2be64b22e84
func AllocOutputContext2(oformat *OutputFormat, formatName, filename string) (*FormatContext, error) {
	var c *C.AVFormatContext
	var oformatPtr *C.AVOutputFormat
	if oformat != nil {
		oformatPtr = oformat.c
	}
	...
	if err := newError(C.avformat_alloc_output_context2(&c, oformatPtr, cFormatName, cFilename)); err != nil {
		return nil, err
	}
	
	return newFormatContextFromC(c), nil
}
...
// https://ffmpeg.org/doxygen/8.0/group__lavf__mux.html#ga18b7b10bb5b94524842de2be64b22e84
func (fc *FormatContext) NewStream(codec *Codec) *Stream {
	var codecPtr *C.AVCodec
	if codec != nil {
		codecPtr = codec.c
	}
	return newStreamFromC(C.avformat_new_stream(fc.c, codecPtr))
}

// GuessFormat guesses the output format based on filename and format name
// https://ffmpeg.org/doxygen/8.0/group__lavf__mux.html#ga18b7b10bb5b94524842de2be64b22e84
func GuessFormat(shortName, filename, mimeType string) *OutputFormat {
	...
	return newOutputFormatFromC(C.av_guess_format(cShortName, cFilename, cMimeType))
}

// GuessCodec guesses the codec based on output format and media type
// https://ffmpeg.org/doxygen/8.0/group__lavf__mux.html#ga18b7b10bb5b94524842de2be64b22e84
func (fc *FormatContext) GuessCodec(shortName, filename, mimeType string, mediaType MediaType) CodecID {
	...
	return CodecID(C.av_guess_codec(fc.c.oformat, cShortName, cFilename, cMimeType, C.enum_AVMediaType(mediaType)))
}

// NetworkInit initializes the network subsystem
// https://ffmpeg.org/doxygen/8.0/group__lavf__core.html#ga18b7b10bb5b94524842de2be64b22e84
func NetworkInit() error {
	return newError(C.avformat_network_init())
}

// NetworkDeinit deinitializes the network subsystem
// https://ffmpeg.org/doxygen/8.0/group__lavf__core.html#ga18b7b10bb5b94524842de2be64b22e84
func NetworkDeinit() error {
	return newError(C.avformat_network_deinit())
}

// https://ffmpeg.org/doxygen/8.0/group__lavf__mux.html#ga18b7b10bb5b94524842de2be64b22e84
func (fc *FormatContext) OpenOutput(filename string, d *Dictionary) error {
	cFilename := C.CString(filename)
	defer C.free(unsafe.Pointer(cFilename))
	return newError(C.avio_open(&fc.c.pb, cFilename, C.AVIO_FLAG_WRITE))
}

// https://ffmpeg.org/doxygen/8.0/group__lavf__mux.html#ga18b7b10bb5b94524842de2be64b22e84
func (fc *FormatContext) CloseOutput() error {
	return newError(C.avio_closep(&fc.c.pb))
}
...
// https://ffmpeg.org/doxygen/8.0/group__lavf__demux.html#ga7b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b
...
// SeekFile seeks to a specific timestamp with more precision
// https://ffmpeg.org/doxygen/8.0/group__lavf__demux.html#ga7b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b
func (fc *FormatContext) SeekFile(streamIndex int, minTs, ts, maxTs int64, flags SeekFlags) error {
	return newError(C.avformat_seek_file(fc.c, C.int(streamIndex), C.int64_t(minTs), C.int64_t(ts), C.int64_t(maxTs), C.int(flags)))
}
...
// QueueAttachedPictures queues attached pictures
// https://ffmpeg.org/doxygen/8.0/group__lavf__demux.html#ga7b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b
func (fc *FormatContext) QueueAttachedPictures() error {
	return newError(C.avformat_queue_attached_pictures(fc.c))
}

// Note: avformat_transfer_internal_stream_timing_info is deprecated in FFmpeg 8.0
